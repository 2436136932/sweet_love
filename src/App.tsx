...
import { Heart, Calendar, Book, CheckSquare, Image as ImageIcon, MessageSquare, User, Link as LinkIcon, Utensils, ChefHat, Droplets, Sparkles } from 'lucide-react';
import { PageType, Anniversary, DiaryEntry, TodoItem, Message, User as UserType, AlbumImage, AlbumOverview, Couple, DailyRating, MenuDish, MealOrderDay, KitchenRecipe, KitchenShoppingList, KitchenCookCheckin, PeriodRecord, PeriodSummary, PeriodDailyLog, PeriodSettings, PeriodOverview } from './types.ts';
import { diaryService, todoService, messageService, authService, anniversaryService, albumService, coupleService, ratingService, menuService, mealOrderService, kitchenService, periodService } from './services/api';
...
function daysSince(date?: string) {
  if (!date) return 0;
  return Math.max(0, Math.floor((Date.now() - new Date(`${date}T00:00:00`).getTime()) / (1000 * 60 * 60 * 24)));
}

function momentLabel(user?: UserType['partner'] | UserType | null) {
  return user?.momentStatusText || (user?.momentStatus ? '已更新此刻状态' : '还没更新');
}
...
function pageFromHash(hash = window.location.hash): PageType {
  ...
  if (!page) return 'home';
  return HASH_ROUTABLE_PAGE_SET.has(page as PageType) ? (page as PageType) : 'home';
}
...
function replaceHashRoute(page: PageType) {
  window.history.replaceState(null, '', `${window.location.pathname}${window.location.search}${pageToHash(page)}`);
}
...
export default function App() {
  ...
  const [shoppingList, setShoppingList] = useState<KitchenShoppingList | null>(null);
  ...
  const loadMoreMessages = useCallback(async (beforeId: string) => {
    if (!user) return;
  ...
  const refreshKitchen = useCallback(async () => {
    ...
    const [recipeData, shoppingData, checkinData, orderData] = await Promise.all([
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [diaryData, todoData, messageData, anniversaryData, albumOverviewData, coupleData, ratingData, dishData, orderData, recipeData, shoppingData, checkinData, periodData] = await Promise.all([
          diaryService.getAll(),
          todoService.getAll(),
          ...
          user.partnerId ? kitchenService.getShoppingList(new Date().toISOString().slice(0, 10)) : Promise.resolve(null),
          user.partnerId ? kitchenService.getCheckins() : Promise.resolve([]),
          user.partnerId ? periodService.getAll() : Promise.resolve({ records: [], logs: [], summary: null, settings: null }),
        ]);
        setDiaries(diaryData);
        setTodos(todoData);
        ...
        showToast(error instanceof Error ? error.message : '数据加载失败，请稍后重试', 'error');
      } finally {
  ...
  useEffect(() => {
    if (!user?.partnerId || !periodSummary || periodSettings?.autoSyncCareTodos === false) return;
    const today = new Date().toISOString().slice(0, 10);
    const leadDays = periodSettings?.reminderLeadDays ?? 3;
    const days = periodSummary.daysUntilNext;
    const shouldSyncPeriod = Boolean(periodSummary.predictedStartDate && days !== undefined && days >= 0 && days <= leadDays);
    ...
    const syncKey = shouldSyncFertility ? `fertility:${today}` : `period:${periodSummary.predictedStartDate}`;
  ...
  useEffect(() => {
    ...
    const refreshCurrentPage = async () => {
      try {
        ...
        showToast(error instanceof Error ? error.message : '页面数据刷新失败', 'error');
      } finally {
        setPageRefreshing(null);
      }
    };

    if (['diary', 'todo', 'album', 'anniversaries', 'menu', 'kitchen', 'period'].includes(currentPage)) {
      void refreshCurrentPage();
    }
  }, [currentPage, refreshAlbumOverview, refreshAnniversaries, refreshDiaries, refreshKitchen, refreshMenu, refreshPeriods, refreshTodos, showToast, user]);
  ...
  const isPageLoading = (page: PageType) => initialDataLoading || pageRefreshing === page;
    ...
    switch (currentPage) {
      ...
      case 'anniversaries': return (
        <Anniversaries 
          ...
          onToggleImportant={async (id) => {
            ...
            await anniversaryService.update(id, { isImportant: !anniversary.isImportant });
            await refreshAnniversaries();
          }}
        />
      );
      ...
      case 'kitchen': return (
        <CoupleKitchen
          ...
          onToggleFavorite={async (recipe) => {
            if (recipe.isFavorite) await kitchenService.unfavoriteRecipe(recipe.id);
          ...
          onGenerateShoppingList={async (date, recipeIds) => {
            const updated = await kitchenService.generateShoppingList(date, recipeIds);
          ...
          onDeleteShoppingItem={async (id) => {
            ...
            setShoppingList((current) => current ? { ...current, items: current.items.filter((item) => item.id !== id) } : current);
          }}
      ...
      case 'period': return (
        <PeriodAssistantPage
          ...
          onBack={goBackOrHome}
          onCreate={async (record) => updatePeriodState(await periodService.create(record))}
          onUpdate={async (id, record) => updatePeriodState(await periodService.update(id, record))}
          onDelete={async (id) => updatePeriodState(await periodService.delete(id))}
          onSaveLog={async (date, log) => updatePeriodState(await periodService.saveLog(date, log))}
          onDeleteLog={async (date) => updatePeriodState(await periodService.deleteLog(date))}
          onUpdateSettings={async (settings) => updatePeriodState(await periodService.updateSettings(settings))}
          onSyncCareTodos={async () => {
            const result = await periodService.syncCareTodos(todayString());
            if (!result.skipped && (result.created > 0 || result.updated > 0)) await refreshTodos();
            return result;
          }}
        />
      );
      case 'profile': return (
        <Profile 
          ...
          onUnbind={async () => {
            if (window.confirm('确定要解除绑定吗？这会清除你们的关联关系。')) {
              try {
                ...
                showToast(error instanceof Error ? error.message : '解除绑定失败', 'error');
              }
            }
          }} 
        />
  ...
  const fixedHeaderPages: PageType[] = ['anniversaries', 'messages', 'menu', 'kitchen', 'album'];
  const usesFixedHeaderLayout = fixedHeaderPages.includes(currentPage);
  const desktopNavItems = [
    { id: 'home', icon: Heart, label: '首页', description: '今日总览' },
    { id: 'diary', icon: Book, label: '日记', description: `${diaries.length} 篇记录` },
    { id: 'album', icon: ImageIcon, label: '相册', description: `${albumOverview?.total || 0} 段回忆` },
    { id: 'todo', icon: CheckSquare, label: '100件', description: `${completedTodoCount}/100 完成` },
    { id: 'anniversaries', icon: Calendar, label: '纪念日', description: `${anniversaries.length} 个日子` },
    { id: 'menu', icon: Utensils, label: '点餐', description: mealOrderQuantity > 0 ? `${mealOrderQuantity} 份待定` : '今天吃什么' },
    { id: 'kitchen', icon: ChefHat, label: '厨房', description: `${kitchenRecipes.length} 道菜谱` },
    { id: 'period', icon: Droplets, label: '姨妈助手', description: periodHint },
    { id: 'messages', icon: MessageSquare, label: '留言', description: `${messages.length} 条心事` },
    { id: 'profile', icon: User, label: '我的', description: user.username },
  ];

  return (
    <div className="app-viewport flex justify-center items-center bg-[#FDF2F8] p-0 md:p-5 xl:p-8">
      {/* Background Decor for Web */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none opacity-40 hidden sm:block">
        <div className="absolute top-[8%] left-[4%] w-96 h-96 bg-pink-200 blur-[150px] rounded-full" />
        <div className="absolute bottom-[8%] right-[6%] w-96 h-96 bg-amber-100 blur-[150px] rounded-full" />
      </div>

      <div className={`app-shell w-full bg-gradient-to-b from-[#FDF2F8] via-[#FEF9F3] to-[#FFF7ED] overflow-hidden relative shadow-2xl font-sans sm:border-[8px] border-white/80 ${isBound ? 'max-w-md md:max-w-[1800px] md:grid md:grid-cols-[260px_minmax(0,1fr)] md:rounded-[2rem] md:border md:bg-white/64 md:backdrop-blur-2xl xl:grid-cols-[300px_minmax(0,1fr)_390px]' : 'max-w-md sm:rounded-[3rem] flex flex-col'}`}>
        {isBound && (
          <aside className="hidden min-h-0 flex-col border-r border-white/70 bg-white/55 px-4 py-5 md:flex">
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block truncate text-sm font-black text-gray-900">{couple?.name || 'SweetLover'}</span>
                <span className="mt-0.5 block text-[11px] font-bold text-gray-400">{daysTogether || 0} 天甜蜜同步</span>
              </span>
            </button>

            <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto pr-1 scrollbar-hide" aria-label="桌面导航">
              {desktopNavItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl transition-colors ${active ? 'bg-white/15 text-pink-100' : 'bg-white/70 text-pink-400 group-hover:text-pink-500'}`}>
                      <Icon size={17} />
                    </span>
                    <span className="min-w-0">
                      <span className="block truncate text-sm font-black">{item.label}</span>
                      <span className={`mt-0.5 block truncate text-[10px] font-bold ${active ? 'text-white/55' : 'text-gray-400'}`}>{item.description}</span>
                    </span>
                  </button>
                );
              })}
            ...
            <div className="mt-5 rounded-[1.5rem] bg-pink-50/70 p-4">
              ...
              <p className="mt-2 text-xs font-bold leading-relaxed text-gray-500">
                {todayRating ? `你给今天打了 ${todayRating.score} 分` : '还没给今天打分'}
        ...
        {/* Main Content Area */}
        <main className={`relative h-full min-h-0 flex-1 overflow-y-auto scrollbar-hide md:col-start-2 md:min-w-0 ${currentPage === 'messages' ? 'xl:col-end-4' : ''}`}>
        ...
        {isBound && currentPage !== 'messages' && (
          <aside className="hidden min-h-0 overflow-y-auto border-l border-white/70 bg-white/45 px-5 py-5 scrollbar-hide xl:block">
            <section className="rounded-[1.75rem] bg-gray-900 p-5 text-white shadow-xl shadow-gray-200">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-pink-200">Love desk</p>
                  <h2 className="mt-2 text-2xl font-black leading-tight">{couple?.name || 'SweetLover'}</h2>
                </div>
                <button
                  type="button"
                  onClick={() => navigateToPage('messages')}
                  className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white/10 text-pink-100 transition-colors hover:bg-white/15"
                  aria-label="打开留言"
                >
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-white/10 p-3">
                  <p className="text-3xl font-black leading-none tabular-nums">{daysTogether || 0}</p>
                  <p className="mt-1 text-[10px] font-bold text-white/55">相伴天数</p>
                </div>
                <div className="rounded-2xl bg-white/10 p-3">
                  <p className="text-3xl font-black leading-none tabular-nums">{pendingTodos.length}</p>
                  <p className="mt-1 text-[10px] font-bold text-white/55">待完成小事</p>
                </div>
              </div>
            </section>
            ...
            <section className="mt-4 rounded-[1.75rem] bg-white/75 p-4 shadow-sm">
              ...
              {[user, user.partner].map((person, index) => (
                <div key={person?.id || index} className="flex items-center gap-3 border-t border-pink-50 py-3 first:border-t-0 first:pt-0 last:pb-0">
                  <AppImage
                    src={person?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(person?.username || 'lover')}`}
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-black text-gray-800">{person?.username || '另一半'}</p>
                    <p className="mt-0.5 truncate text-[11px] font-bold text-gray-400">{momentLabel(person)}</p>
                  </div>
                  <span className={`h-2.5 w-2.5 rounded-full ${index === 0 ? 'bg-pink-500' : 'bg-blue-400'}`} />
                </div>
              ))}
            </section>
            ...
            <section className="mt-4 rounded-[1.75rem] bg-white/75 p-4 shadow-sm">
              ...
              <div className="mt-3 space-y-2">
                ...
                >
                  <span>
                    <span className="block text-xs font-black text-gray-800">恋爱清单</span>
                    <span className="mt-0.5 block text-[10px] font-bold text-gray-400">{completedTodoCount}/100 已完成</span>
                  </span>
                ...
                >
                  <span>
                    <span className="block text-xs font-black text-gray-800">姨妈助手</span>
                    <span className="mt-0.5 block text-[10px] font-bold text-gray-400">{periodHint}</span>
                  </span>
                ...
                >
                  <span>
                    <span className="block text-xs font-black text-gray-800">今天吃什么</span>
                    <span className="mt-0.5 block text-[10px] font-bold text-gray-400">{mealOrderQuantity > 0 ? `${mealOrderQuantity} 份已加入` : '去厨房看看'}</span>
                  </span>
            ...
            {nextAnniversary && (
              <section className="mt-4 rounded-[1.75rem] bg-white/75 p-4 shadow-sm">
                <p className="text-[10px] font-black uppercase tracking-[0.18em] text-gray-400">Next date</p>
                <h3 className="mt-2 truncate text-sm font-black text-gray-900">{nextAnniversary.title}</h3>
                <p className="mt-1 text-xs font-bold text-gray-400">{nextAnniversary.date}</p>
              </section>
            )}
          </aside>
        )}
        ...
        {user?.partnerId && currentPage !== 'messages' && currentPage !== 'period' && (
          <nav className="app-bottom-nav absolute left-1/2 -translate-x-1/2 w-[90%] bg-white/60 backdrop-blur-2xl border border-white/50 h-16 rounded-[2rem] shadow-xl flex items-center justify-around px-2 z-50 md:hidden">
            {mobileNavItems.map((item) => (
              ...
              >
                <item.icon size={20} />
                <span className="text-[10px] mt-0.5 font-black uppercase tracking-tighter">{item.label}</span>
              </button>
            ))}
          </nav>
        )}
