...
export default function CoupleKitchen({
  ...
  onAddOrderItem: (
    dish: ReturnType<typeof recipeToTicketDish>
  ) => Promise<void> | void
  onUpdateOrderItem: (
    id: string,
  ...
  const [pendingRecipeActions, setPendingRecipeActions] = useState<
  ...
  const shoppingProgress =
    shoppingItems.length > 0
      ? Math.round((checkedShoppingCount / shoppingItems.length) * 100)
      : 0
  ...
  const uploadImage = async (
    ...
    try {
      ...
      if (target === 'recipe')
        setRecipeForm((prev) => ({ ...prev, imageUrl: url }))
      if (target === 'checkin')
        setCheckinForm((prev) => ({ ...prev, imageUrl: url }))
  ...
  const generateKitchenIdeas = async () => {
    ...
    try {
      const result = await aiService.generate({
        type: 'kitchen_ideas',
        prompt:
          aiKitchenPrompt.trim() ||
          '今天不知道吃什么，给我们几个适合情侣一起做的家常建议。',
        context: {
  ...
  const generateAiRecipe = async () => {
    ...
    try {
      ...
      showToast(
        error instanceof Error ? error.message : 'AI 菜谱生成失败，请稍后重试',
        'error'
      )
    } finally {
      setAiLoading(null)
  ...
  const adoptAiRecipeDraft = () => {
    ...
    setRecipeForm({
      ...
      category: RECIPE_CATEGORIES.includes(aiRecipeDraft.category)
        ? aiRecipeDraft.category
        : RECIPE_CATEGORIES[0],
      summary: aiRecipeDraft.summary,
      difficulty: DIFFICULTY_OPTIONS.includes(aiRecipeDraft.difficulty)
        ? aiRecipeDraft.difficulty
        : DIFFICULTY_OPTIONS[0],
      cookTime: String(aiRecipeDraft.cookTime || 30),
      servings: String(aiRecipeDraft.servings || 2),
      ingredients:
        aiRecipeDraft.ingredients.length > 0
          ? aiRecipeDraft.ingredients
          : [blankIngredient()],
  ...
  const closeRecipeFormModal = useModalHistory(
    'kitchen-recipe-form',
    showRecipeForm,
    closeRecipeForm
  )
  const closeCheckinFormModal = useModalHistory(
    'kitchen-checkin-form',
    showCheckinForm,
    () => setShowCheckinForm(false)
  )
  const closeDetailRecipeModal = useModalHistory(
    'kitchen-recipe-detail',
    Boolean(detailRecipe),
    () => setDetailRecipeId(null)
  )
  const closeAiPanel = useModalHistory('kitchen-ai', aiPanelOpen, () =>
    setAiPanelOpen(false)
  )
  ...
  const submitRecipe = async () => {
    ...
    try {
      ...
      if (
        ...
        showToast(
          recipeFormMode === 'copy' ? '已保存为我的菜谱' : '菜谱已保存',
          'success'
        )
      }
      closeRecipeForm()
  ...
  const isRecipeActionPending = (recipeId: string, action: RecipeAction) =>
    Boolean(pendingRecipeActions[`${recipeId}:${action}`])
  ...
  const handleAddOrderItem = async (recipe: KitchenRecipe) => {
    ...
    try {
      ...
      showToast(
        error instanceof Error ? error.message : '加入小票失败，请稍后重试',
        'error'
      )
    } finally {
  ...
  const handleBuyIngredients = (recipe: KitchenRecipe) => {
    ...
    showToast(
      alreadySelected
        ? `「${recipe.title}」已在买菜清单候选中`
        : `已选中「${recipe.title}」，可以生成买菜清单`,
      'success'
    )
  }
  ...
  const deleteShoppingItem = async (item: KitchenShoppingListItem) => {
    ...
    try {
      ...
      showToast(
        error instanceof Error ? error.message : '删除食材失败，请稍后重试',
        'error'
      )
    } finally {
  ...
  const handleDeleteRecipe = async (id: string) => {
    ...
    try {
      ...
      showToast(
        error instanceof Error ? error.message : '删除菜谱失败',
        'error'
      )
    }
  }
  ...
  const handleDeleteCheckin = async (id: string) => {
    ...
    try {
      ...
      showToast(
        error instanceof Error ? error.message : '删除打卡记录失败',
        'error'
      )
    }
  }
  ...
  const updateIngredient = (
    index: number,
    patch: Partial<KitchenIngredient>
  ) => {
    setRecipeForm((prev) => ({
      ...prev,
    ...
    setRecipeForm((prev) => ({
      ...
      ingredients:
        prev.ingredients.length > 1
          ? prev.ingredients.filter((_, itemIndex) => itemIndex !== index)
          : [blankIngredient()]
    }))
  }
  ...
  const renderShoppingItem = (
    ...
    >
      ...
      >
        <span
          className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl ${purchased ? 'bg-emerald-50 text-emerald-500' : 'bg-rose-50 text-rose-400'}`}
        >
          <Check size={16} />
        </span>
        <span className="min-w-0 flex-1">
          <span
            className={`block text-sm font-black ${purchased ? 'text-gray-300 line-through' : 'text-gray-900'}`}
          >
            {item.name}
          </span>
        ...
        {!purchased && (
          <span className="rounded-full bg-gray-100 px-2 py-1 text-[9px] font-black text-gray-400">
            待买
          </span>
        )}
      </button>
  ...
  return (
    <div className="relative flex h-full min-h-0 flex-col overflow-hidden bg-[#FFF8F2]">
      <header
        className={`${activeTab === 'recipes' ? 'shrink-0 lg:sticky lg:top-0 lg:z-30' : 'sticky top-0 z-30 shrink-0'} border-b border-white/70 bg-[linear-gradient(135deg,#fff7ed_0%,#fff1f2_48%,#effdf5_100%)] px-4 pb-3 pt-6 shadow-sm`}
      >
        <div className="mb-3 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-xl bg-accent text-on-accent shadow-sm transition-transform active:scale-95"
            aria-label="新增菜谱"
          >
            <Plus size={18} />
          </button>
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() =>
                  aiPanelOpen ? closeAiPanel() : setAiPanelOpen(true)
                }
              ...
              <button
                ...
                className="flex h-[34px] shrink-0 items-center justify-center gap-1.5 rounded-xl bg-accent px-3 text-[10px] font-black text-on-accent shadow-sm disabled:opacity-40"
              >
                <ListChecks size={14} />
                {busy
                  ? '生成中'
                  : selectedRecipeIds.length > 0
                    ? `买菜 (${selectedRecipeIds.length})`
                    : '生成买菜'}
              </button>
            </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      active
                        ? 'bg-accent text-on-accent shadow-sm shadow-black/10'
                ...
                >
                  ...
                  <div className="p-3.5 flex flex-col flex-1 min-w-0">
                    ...
                    <div className="mt-3.5 flex items-center justify-between gap-2">
                      ...
                      >
                        {isRecipeActionPending(recipe.id, 'ticket')
                          ? '...'
                          : '+ 小票'}
                      </button>
                    </div>
      ...
      {activeTab === 'shopping' && (
        <main className="flex-1 overflow-y-auto px-5 pb-28 pt-4 scrollbar-hide">
          <section className="overflow-hidden rounded-[30px] bg-accent text-on-accent shadow-xl shadow-rose-100/50">
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-[11px] font-bold text-white/55">
                    {shoppingItems.length} 项 · 已买 {checkedShoppingCount} 项 ·
                    待买 {pendingShoppingItems.length} 项
                  </p>
                </div>
                <label className="rounded-2xl bg-white/10 px-3 py-2 text-[10px] font-black text-white/80">
                  ...
                  <input
                    ...
                    onBlur={(event) =>
                      void onLoadShoppingList(event.target.value)
                    }
          ...
          <section className="mt-4 rounded-[28px] bg-white/85 p-4 shadow-sm ring-1 ring-white/80">
            ...
            <div className="mt-3 flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              {hasSelectedRecipes ? (
                selectedRecipes.map((recipe) => (
                  <span
                    key={recipe.id}
                    className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-emerald-50 py-1.5 pl-3 pr-1.5 text-[10px] font-black text-emerald-600"
                  >
                    {recipe.title}
                    <button
                      type="button"
                ...
                <span className="rounded-full bg-gray-100 px-3 py-1.5 text-[10px] font-black text-gray-400">
                  还没选菜谱
                </span>
              )}
            </div>
            <button
              ...
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-accent py-3 text-xs font-black text-on-accent shadow-lg shadow-rose-100 transition-all active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-40 disabled:shadow-none"
            >
              <ListChecks size={15} />
              {busy
                ? '生成中...'
              ...
              {!hasSelectedRecipes ? (
                <>
                  ...
                  <p className="mt-1 text-[10px] font-bold leading-relaxed text-gray-400">
                    选好菜谱后，这里会帮你把食材合并成一份采购清单。
                  </p>
                  <button
                    type="button"
                    onClick={() => setActiveTab('recipes')}
                    className="mt-4 rounded-2xl bg-accent px-5 py-2.5 text-[10px] font-black text-on-accent transition-all active:scale-[0.97]"
                  >
                    去选菜谱
                  </button>
                </>
                ...
                <div>
                  <div className="mb-2 flex items-center justify-between px-1">
                    ...
                    <span className="text-[10px] font-black text-emerald-500">
                      {purchasedShoppingItems.length} 项
                    </span>
                  </div>
                  <div className="space-y-2">
                    {purchasedShoppingItems.map((item) =>
                      renderShoppingItem(item, true)
                    )}
                  </div>
                </div>
              )}
      ...
      {activeTab === 'checkins' && (
        <main className="flex-1 overflow-y-auto px-5 pb-28 pt-4 scrollbar-hide">
          <button
            onClick={() => openCheckin()}
            className="mb-4 flex w-full items-center justify-between rounded-[28px] bg-accent p-4 text-left text-on-accent shadow-lg shadow-rose-100"
          >
            <span>
          ...
          <div className="space-y-3">
            {checkins.length === 0 ? (
              ...
              checkins.map((checkin) => (
                <article
                  key={checkin.id}
                  className="overflow-hidden rounded-[28px] bg-white/85 shadow-sm ring-1 ring-white/80"
                >
                  {checkin.imageUrl && (
                    <AppImage
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-rose-400">
                          {checkin.date} · {checkin.user?.username || '我们'}
                        </p>
                      ...
                      <button
                        ...
                        className="rounded-2xl bg-red-50 p-2 text-red-300"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
                    ...
                    {checkin.rating && (
                      <p className="mt-3 text-[10px] font-black text-amber-500">
      ...
      {activeTab === 'ticket' && (
        <main className="flex-1 overflow-y-auto px-5 pb-28 pt-4 scrollbar-hide">
          <section className="rounded-[28px] bg-white/85 p-4 shadow-sm ring-1 ring-white/80">
            ...
            <p className="mt-1 text-[10px] font-bold text-gray-400">
              {order?.date || today()} · 共 {totalQuantity} 份
            </p>
          </section>
            ...
            {orderItems.length === 0 ? (
              ...
              orderItems.map((item) => (
                <TicketItem
                  ...
                  item={item}
                  onIncrement={() =>
                    onUpdateOrderItem(item.id, { quantity: item.quantity + 1 })
                  }
          ...
          <Modal onClose={closeRecipeFormModal}>
            ...
            <div className="mt-5 space-y-4">
              <ImageUpload
                ...
                onChange={(event) => uploadImage(event, 'recipe')}
              />
              ...
              <section className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="rounded-full bg-rose-50 px-3 py-1 text-[10px] font-black text-rose-500"
                  >
                    添加
                  </button>
                </div>
                <div className="space-y-2">
                  {recipeForm.ingredients.map((ingredient, index) => (
                    <div
                      key={index}
                      className="rounded-3xl bg-white/70 p-3 shadow-sm ring-1 ring-white/80"
                    >
                      <div className="grid grid-cols-[1fr_72px_64px_36px] gap-2">
                        ...
                        <button
                          ...
                          className="flex h-11 items-center justify-center rounded-2xl bg-red-50 text-red-300 transition-all active:scale-95"
                        >
                          <X size={15} />
                        </button>
                      </div>
                      <input
                        ...
                        onChange={(event) =>
                          updateIngredient(index, { note: event.target.value })
                        }
                        placeholder="备注，可不填"
              ...
              <section className="space-y-2">
                ...
                <div className="space-y-2">
                  {recipeForm.steps.map((step, index) => (
                    ...
                    >
                      <span className="mt-2 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-accent text-[10px] font-black text-on-accent">
                        {index + 1}
                      </span>
                      <textarea
                        value={step}
                      ...
                      <button
                        ...
                        className="mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-red-50 text-red-300 transition-all active:scale-95"
                      >
                        <X size={15} />
                      </button>
                    </div>
              ...
              <button
                ...
                disabled={!recipeForm.title.trim() || busy}
                className="w-full rounded-2xl bg-rose-500 py-4 text-sm font-black text-white shadow-lg shadow-rose-100 transition-all active:scale-[0.99] disabled:opacity-40 disabled:shadow-none"
              >
                {busy
                  ? '保存中...'
                  : recipeFormMode === 'edit'
          ...
          <Modal onClose={closeCheckinFormModal}>
            ...
            <div className="mt-5 space-y-3">
              <ImageUpload
                imageUrl={checkinForm.imageUrl}
                uploading={imageUploading}
                onChange={(event) => uploadImage(event, 'checkin')}
              />
              <input
                type="date"
              ...
              />
              <input
                value={checkinRecipe?.title || checkinForm.title}
                onChange={(event) =>
                  setCheckinForm((prev) => ({
                    ...prev,
                    title: event.target.value
                ...
                disabled={Boolean(checkinRecipe)}
                placeholder="今天做了什么"
                className="field disabled:text-gray-400"
              />
              <textarea
              ...
              <div className="flex justify-center gap-1 rounded-2xl bg-white/80 p-3">
                {[1, 2, 3, 4, 5].map((rating) => (
                  <button
                    key={rating}
                    onClick={() =>
                      setCheckinForm((prev) => ({ ...prev, rating }))
                    }
                    className={
                      rating <= checkinForm.rating
                        ? 'text-amber-400'
              ...
              <button
                ...
                disabled={busy}
                className="w-full rounded-2xl bg-rose-500 py-4 text-sm font-black text-white disabled:opacity-40"
              >
                保存打卡
              </button>
            </div>
          ...
          <Modal onClose={closeDetailRecipeModal}>
            <div className="relative -mx-7 -mt-7 mb-5 h-48 bg-rose-50 overflow-hidden rounded-t-[32px]">
              {detailRecipe.imageUrl ? (
                ...
                <div className="flex h-full w-full flex-col items-center justify-center bg-gradient-to-br from-rose-50 via-white to-amber-50 text-center">
                  <ChefHat
                    ...
                    className="text-rose-200"
                  />
                  <span className="text-xs font-black text-gray-400 mt-1">
                    暂无图片
                  </span>
                </div>
              )}
              ...
              >
                <Heart
                  size={18}
                  className={detailRecipe.isFavorite ? 'fill-current' : ''}
                />
              </button>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <span className="flex-1 rounded-2xl bg-rose-50/60 p-2 text-center shadow-sm">
                  ...
                  <span className="mt-0.5 block text-xs font-black text-rose-600">
                    {detailRecipe.cookTime || 20} 分钟
                  </span>
                </span>
                <span className="flex-1 rounded-2xl bg-emerald-50/60 p-2 text-center shadow-sm">
                  ...
                  <span className="mt-0.5 block text-xs font-black text-emerald-600">
                    {detailRecipe.servings || 2} 人份
                  </span>
                </span>
                <span className="flex-1 rounded-2xl bg-amber-50/60 p-2 text-center shadow-sm">
                  ...
                  <span className="mt-0.5 block text-xs font-black text-amber-600">
                    {detailRecipe.difficulty || '简单'}
                  </span>
                </span>
              </div>
              ...
              {/* Ingredients List */}
              <div>
                <h3 className="mb-2 text-xs font-black uppercase tracking-wider text-rose-500">
                  所需食材
                </h3>
                ...
                detailRecipe.ingredients.length > 0 ? (
                  <div className="divide-y divide-gray-100 rounded-3xl bg-white p-3 ring-1 ring-white/70 shadow-sm max-h-48 overflow-y-auto">
                    {detailRecipe.ingredients.map((item, index) => (
                      <div
                        key={index}
                        className="flex justify-between py-1.5 text-xs first:pt-0 last:pb-0"
                      >
                ...
                {detailRecipe.steps && detailRecipe.steps.length > 0 ? (
                  <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                    {detailRecipe.steps.map((step, index) => (
                      <div
                        key={index}
                        className="flex gap-2.5 rounded-2xl bg-white/60 p-3 ring-1 ring-white/80 shadow-sm"
                      >
                        <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-accent text-[10px] font-black text-on-accent">
                          {index + 1}
                        </span>
              ...
              <div className="pt-2 space-y-2">
                <button
                  ...
                  className="w-full flex min-h-11 items-center justify-center rounded-2xl bg-accent text-xs font-black text-on-accent shadow-md transition-all active:scale-[0.98]"
                >
                  {isRecipeActionPending(detailRecipe.id, 'ticket')
                    ? '加入中...'
                    : '加入今日小票'}
                </button>
            ...
            <div className="mt-4 flex rounded-2xl bg-rose-50/50 p-1 ring-1 ring-rose-100/30">
              ...
              >
                <Sparkles
                  size={13}
                  className={
                    aiTab === 'recipe' ? 'text-rose-500' : 'text-gray-400'
                  }
                />
                AI 菜谱
              </button>
              ...
              >
                <Utensils
                  size={13}
                  className={
                    aiTab === 'ideas' ? 'text-rose-500' : 'text-gray-400'
                  }
                />
                吃什么灵感
              </button>
              ...
              {aiTab === 'recipe' ? (
                <div className="space-y-3">
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="relative">
                        ...
                        />
                        <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[10px] font-black text-gray-400">
                          人份
                        </span>
                      </div>
                      <div className="relative">
                  ...
                  <button
                    ...
                    className="flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-accent text-xs font-black text-on-accent shadow-md transition-all active:scale-[0.99] disabled:opacity-50"
                  >
                    {aiLoading === 'recipe' ? (
                    ...
                    <div className="mt-4 rounded-3xl bg-rose-50/40 p-4 border border-rose-100/50">
                      {aiRecipeDraft.imageUrl ? (
                        ...
                        <div className="mb-3 flex h-28 w-full flex-col items-center justify-center rounded-2xl border border-dashed border-rose-100 bg-white/50 text-center">
                          <ChefHat
                            size={20}
                            className="mb-1 text-rose-300 animate-pulse"
                          />
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-1 text-[10px] font-black text-rose-500">
                            {aiRecipeDraft.category} ·{' '}
                            {aiRecipeDraft.difficulty} ·{' '}
                            {aiRecipeDraft.cookTime}分钟 ·{' '}
                            {aiRecipeDraft.servings}人份
                          </p>
                        </div>
                        <button
                          type="button"
                      ...
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {aiRecipeDraft.ingredients
                          ...
                          .map((ingredient, idx) => (
                            <span
                              key={idx}
                              className="max-w-full truncate rounded-full bg-white px-2.5 py-1 text-[9px] font-bold text-rose-500 border border-rose-100/30"
                            >
                              {ingredient.name}
                              {ingredient.amount
                ...
                <div className="space-y-3">
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      onChange={(event) =>
                        setAiKitchenPrompt(event.target.value)
                      }
                      rows={3}
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className="mt-0.5 text-[10px] font-bold text-rose-400">
                {item.category} · {item.user?.username || '我们'}
              </p>
            </div>
            <button
              onClick={onDelete}
              className="rounded-xl bg-red-50 p-2 text-red-300"
            >
              <Trash2 size={14} />
            </button>
          </div>
          <div className="mt-3 flex items-center gap-2">
            ...
            <span className="min-w-5 text-center text-sm font-black text-gray-800">
              {item.quantity}
            </span>
            <button
              onClick={onIncrement}
  ...
  return (
    <div className="fixed inset-0 z-[180] flex items-end justify-center p-4">
      ...
      <motion.div
        ...
        className="relative max-h-[86vh] w-full max-w-sm overflow-y-auto rounded-[32px] bg-[#FFF8F2] p-7 shadow-2xl ring-1 ring-white/70 scrollbar-hide"
      >
        <button
          onClick={onClose}
          className="absolute right-6 top-6 rounded-full bg-white p-2 text-gray-300 shadow-sm"
        >
          <X size={20} />
        </button>
        {children}
