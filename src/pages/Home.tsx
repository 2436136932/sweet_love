...
import { Heart, Calendar, MessageCircle, Star, ChevronRight, CheckSquare, Book, Sparkles, Send, Utensils, Briefcase, Moon, SmilePlus, BatteryLow, Laptop, X, Video } from 'lucide-react';
import type { LucideIcon } from 'lucide-react';
import { PageType, Anniversary, TodoItem, Couple, DailyRating, User, MealOrderDay, AlbumImage, PeriodRecord, PeriodSummary, MomentStatus, PublicUser } from '../types';
...
const momentStatusOptions: Array<{
  ...
  { key: 'missing', label: '想你了', fallback: '想你了', icon: Heart, color: 'bg-pink-100 text-pink-600 border-pink-100' },
  { key: 'busy', label: '在忙', fallback: '在忙，晚点回', icon: Briefcase, color: 'bg-amber-100 text-amber-600 border-amber-100' },
  { key: 'resting', label: '休息中', fallback: '在休息', icon: Moon, color: 'bg-indigo-100 text-indigo-600 border-indigo-100' },
  { key: 'happy', label: '开心', fallback: '心情很好', icon: SmilePlus, color: 'bg-emerald-100 text-emerald-600 border-emerald-100' },
  { key: 'tired', label: '有点累', fallback: '需要抱抱', icon: BatteryLow, color: 'bg-slate-100 text-slate-600 border-slate-100' },
  { key: 'eating', label: '吃饭中', fallback: '正在吃饭', icon: Utensils, color: 'bg-orange-100 text-orange-600 border-orange-100' },
  { key: 'working', label: '工作中', fallback: '专心工作中', icon: Laptop, color: 'bg-blue-100 text-blue-600 border-blue-100' },
];
...
export default function Home({ 
  ...
  const [momentStatus, setMomentStatus] = useState<MomentStatus>(user.momentStatus || 'missing');
  const [momentStatusText, setMomentStatusText] = useState(user.momentStatusText || '');
  ...
  const closeMomentPanel = useModalHistory('home-moment', momentPanelOpen, () => setMomentPanelOpen(false));
  const closeLoveAiPanel = useModalHistory('home-ai', loveAiOpen, () => setLoveAiOpen(false));
  const myRating = useMemo(() => dailyRatings.find((rating) => rating.userId === user.id), [dailyRatings, user.id]);
  const partnerRating = useMemo(() => dailyRatings.find((rating) => rating.userId !== user.id), [dailyRatings, user.id]);
  ...
  const stats = {
    days: couple?.startDate ? Math.max(0, Math.floor((new Date().getTime() - new Date(couple.startDate + 'T00:00:00').getTime()) / (1000 * 60 * 60 * 24))) : 520,
  };

  const pendingTodos = todos.filter(t => !t.completed);
  const completedTodoCount = todos.filter(t => t.completed).length;
  const todoGoalTotal = 100;
  const todoProgress = Math.min(100, Math.round((completedTodoCount / todoGoalTotal) * 100));
  const nextTodo = pendingTodos[0];
  const nearestAnniversary = useMemo(() => {
    if (anniversaries.length === 0) return null;
    return [...anniversaries].sort((a, b) => getRemainingDays(a.date) - getRemainingDays(b.date))[0];
  }, [anniversaries]);
  const anniversaryDaysLeft = nearestAnniversary ? getRemainingDays(nearestAnniversary.date) : null;
  const importantAnniversaryCount = anniversaries.filter((anniversary) => anniversary.isImportant).length;
  const mealOrderQuantity = mealOrder?.items.reduce((sum, item) => sum + item.quantity, 0) || 0;
  ...
  const scrollToRating = () => {
    document.getElementById('daily-rating')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };
  const dailyLoveTasks = [
    ...
    },
    {
      ...
      description: mealOrderQuantity > 0
    ...
    },
    {
      ...
      description: pendingTodos.length > 0
  ...
  const periodFocus = useMemo(() => {
    ...
    return {
      title: daysUntilNext === 0 ? '预计今天开始' : `预计还有 ${daysUntilNext} 天`,
    ...
    : periodNeedsAttention
      ? {
          ...
          title: periodSummary?.isInPeriod
            ? '照顾她的今天'
            : (periodSummary?.daysUntilNext !== undefined && periodSummary.daysUntilNext < 0)
              ? '经期已推迟'
              : '经期快到了',
      ...
      : pendingTodos.length > 0
        ...
        : {
            label: '今日重点',
            title: '一起决定吃什么',
            detail: mealOrderQuantity > 0 ? `小票里已有 ${mealOrderQuantity} 份` : '去厨房找一个今晚菜谱',
            icon: Utensils,
            onClick: () => onNavigate('kitchen'),
          };
  ...
  const handleRatingSubmit = async () => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : '评分保存失败，请稍后重试', 'error');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleMomentSave = async () => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : '此刻状态保存失败，请稍后重试', 'error');
    } finally {
  ...
  const handleMomentClear = async () => {
    setMomentSaving(true);
    try {
      const updatedUser = await userService.updateMomentStatus({ momentStatus: null, momentStatusText: '' });
      onUpdateUser(updatedUser);
      ...
      showToast(error instanceof Error ? error.message : '此刻状态清除失败，请稍后重试', 'error');
    } finally {
      setMomentSaving(false);
    }
  };
  ...
  const handleLoveAiGenerate = async () => {
    const prompt = loveAiPrompt.trim() || '帮我写一句今天可以发给另一半的温柔留言';
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : 'AI 生成失败，请稍后重试', 'error');
    } finally {
    ...
    <div className="px-6 py-8 lg:px-8 lg:py-7">
      ...
      <header className="mb-8 flex items-center justify-between lg:hidden">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-gray-800 tracking-tight">{couple?.name || 'SweetLover'}</h1>
        </div>
        <button
          type="button"
          aria-label="打开留言"
      ...
      <section className="mb-5 lg:mb-6">
        <div className="relative h-[210px] overflow-hidden rounded-[30px] bg-gradient-to-br from-pink-100 via-white to-amber-100 shadow-lg shadow-pink-100/50 lg:h-[300px] lg:rounded-[34px]">
          {couple?.coverImage && (
            <AppImage src={couple.coverImage} className="h-full w-full object-cover" alt="Cover" width={960} height={540} crop="cover" priority />
          )}
          ...
          <div className="absolute right-3 top-3 rounded-[22px] bg-white px-3.5 py-2.5 text-center shadow-lg shadow-gray-900/10">
            {couple?.startDate ? (
              <>
                <p className="text-2xl font-black leading-none text-pink-500">{stats.days}</p>
                <p className="mt-1 text-[10px] font-black text-gray-500">天</p>
              </>
            ) : (
              <button
                onClick={() => onNavigate('profile')}
                className="flex flex-col items-center justify-center gap-1 text-pink-500"
              >
                <Calendar size={16} />
          ...
          <div className="absolute bottom-5 left-5 right-5 text-white">
            ...
            <p className="truncate text-3xl font-black leading-tight drop-shadow-sm lg:text-4xl">{couple?.name || 'SweetLover'}</p>
            <p className="mt-1.5 text-sm font-bold text-white/90">第{stats.days}天 · 一直甜甜的</p>
          </div>
        </div>
        ...
        <div className="mt-3 rounded-[28px] border border-white/80 bg-white/80 p-3 shadow-sm backdrop-blur-md">
          <div className="mb-1.5 flex items-center justify-between gap-3 px-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-black text-gray-800">此刻状态</span>
            </div>
            <button
              type="button"
        ...
        {momentPanelOpen && (
          ...
          >
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.22em] text-pink-400">Moment</p>
                <h3 className="text-sm font-black text-gray-800">设置此刻状态</h3>
              </div>
              <button
                type="button"
                ...
                return (
                  <button
                    ...
                    className={`min-h-[58px] rounded-2xl border px-1.5 py-2 text-[10px] font-black transition-all ${
                      active ? `${option.color} shadow-sm ring-2 ring-white` : 'border-white bg-white/75 text-gray-500'
                    }`}
                  >
            ...
            <div className="mt-3">
              <div className="mb-1 flex items-center justify-between">
                <label className="text-xs font-black text-gray-500" htmlFor="moment-status-text">一句短语</label>
                <span className="text-[10px] font-bold text-gray-300">{momentStatusText.length}/30</span>
              </div>
              <input
                id="moment-status-text"
                type="text"
                value={momentStatusText}
                onChange={(event) => setMomentStatusText(event.target.value.slice(0, 30))}
            ...
            <div className="mt-3 grid grid-cols-[0.8fr_1.2fr] gap-2">
              <button
                ...
                disabled={momentSaving || (!user.momentStatus && !user.momentStatusText)}
              ...
              >
                {momentSaving ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white/40 border-t-white" /> : <Sparkles size={14} />}
                保存状态
              </button>
            </div>
          </motion.div>
      ...
      <section className="mb-6 space-y-3">
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-[10px] font-black text-white/50">{primaryFocus.label}</span>
            <span className="mt-0.5 block truncate text-base font-black">{primaryFocus.title}</span>
            <span className="mt-1 block truncate text-xs font-bold text-white/60">{primaryFocus.detail}</span>
          </span>
          ...
          {secondaryActions.map((action) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-2 text-[10px] font-black text-gray-400">{action.label}</p>
                <p className="mt-0.5 truncate text-xs font-black text-gray-800">{action.title}</p>
              </button>
            );
          })}
        </div>
        ...
        {loveAiOpen && (
          ...
          >
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                <h2 className="text-base font-black text-gray-800">恋爱助手</h2>
                <p className="mt-1 text-[10px] font-bold text-gray-400">留言、约会、安慰话术都可以先生成草稿</p>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-3">
              ...
              >
                {loveAiLoading ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white/40 border-t-white" /> : <Sparkles size={14} />}
                问问 AI
              </button>
              {loveAiResult && (
      ...
      {photoWallItems.length > 0 && (
        <section className="mb-6">
          <div className="mb-3 flex items-center justify-between">
            <div>
              <h2 className="text-base font-bold text-gray-800">精选照片墙</h2>
              <p className="mt-1 text-[10px] font-bold text-gray-400">从相册里挑出来的今日回忆</p>
            </div>
            <button
          ...
          <div 
            ...
            style={{
              backgroundImage: 'radial-gradient(#E8E2D9 1.1px, transparent 1.1px)',
          ...
          >
            ...
            <div className="mx-auto size-[208px] shrink-0 sm:size-[232px] relative z-10">
              <Stack
                ...
                cards={photoWallItems.map((item) => (
                  <div key={item.id} className="relative size-full flex flex-col bg-white p-2.5 pb-7 shadow-md border border-gray-100 rounded-[2px] text-left">
                    {/* 照片区域 */}
                    <div className="relative flex-1 w-full overflow-hidden bg-pink-50 rounded-sm">
                      {item.mediaType === 'video' ? (
                        <video src={buildImageUrl(item.src)} className="size-full object-cover" muted playsInline />
                      ) : (
                        <AppImage src={item.src} alt={item.title} className="size-full object-cover" width={464} height={464} crop="square" sizes="232px" referrerPolicy="no-referrer" />
                      )}
                      {item.mediaType === 'video' && (
            ...
            <div className="min-w-0 text-center relative z-10">
              <p className="text-sm font-black text-gray-800">拖一拖，翻到下一张回忆</p>
              <p className="mt-1 text-xs font-bold leading-relaxed text-gray-400">
                {photoWallItems.length} 张精选会自动轮播，也可以点击照片切换。
              </p>
              <button
                type="button"
                onClick={() => onNavigate('album')}
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="relative z-10 flex h-full flex-col">
            <div className="flex items-center justify-between gap-2 mb-4">
              ...
              <span className="rounded-full bg-white/60 px-2 py-1 text-[9px] font-black text-purple-500">
                {importantAnniversaryCount > 0 ? `${importantAnniversaryCount} 个重要` : '纪念日'}
              </span>
            </div>
            <span className="text-[10px] font-bold text-purple-600 uppercase tracking-wider">下一个纪念日</span>
            ...
            {nearestAnniversary ? (
              <div className="mt-auto flex items-end justify-between gap-2 pt-4">
                <div>
                  <p className="text-[10px] font-bold text-gray-400">{nearestAnniversary.date}</p>
                  <p className="mt-0.5 text-xs font-bold text-gray-500">快到啦</p>
                </div>
                <div className="text-right">
                  <span className="text-3xl font-black leading-none text-purple-500">{anniversaryDaysLeft}</span>
                  <span className="ml-1 text-xs font-black text-purple-300">天</span>
                </div>
              </div>
            ) : (
              <div className="mt-auto pt-4">
                <p className="text-xs font-bold leading-relaxed text-gray-400">记录相识、生日、第一次约会这些值得记住的日子</p>
              </div>
            )}
          </div>
        ...
        >
          <div className="relative z-10 flex h-full flex-col">
            ...
            <span className="text-[10px] font-bold text-pink-600 uppercase tracking-wider">100 件恋爱小事</span>
            <p className="mt-1 line-clamp-1 text-sm font-black text-gray-800">
              {nextTodo ? nextTodo.title : todos.length > 0 ? '今天全部完成啦' : '添加第一个小愿望'}
            </p>
            <div className="mt-auto pt-4">
              <div className="mb-2 flex items-center justify-between text-[10px] font-black">
                <span className="text-gray-400">{pendingTodos.length > 0 ? `待完成 ${pendingTodos.length} 个` : '没有待完成'}</span>
      ...
      <section className="mb-6">
        <div className="flex items-end justify-between gap-3 mb-4">
          <div>
            ...
            <p className="mt-1 text-[10px] font-bold text-gray-400">不是普通待办，是今天可以一起完成的小互动</p>
          </div>
          <button
            onClick={() => onNavigate('todo')}
          ...
          {dailyLoveTasks.map((task, index) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-3 truncate text-sm font-black text-gray-800">{task.title}</p>
                <p className="mt-1 line-clamp-2 min-h-[30px] text-[10px] font-bold leading-relaxed text-gray-400">{task.description}</p>
                <span className={`mt-3 inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-black ${task.done ? 'bg-gray-100 text-gray-500' : 'bg-pink-500 text-white'}`}>
                  {task.actionLabel}
      ...
      <section id="daily-rating" className="mb-10 scroll-mt-6">
        <motion.div 
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-black text-gray-800">今日情侣评分</h3>
            </div>
          ...
          <div className="space-y-4">
            <div className="p-3 bg-pink-50/50 rounded-2xl border border-pink-100/30">
              <p className="text-xs text-pink-600 font-bold text-center">
                {myRating ? '想改一下今天的幸福指数吗？' : '今天我们的幸福指数是多少？'}
              </p>
            </div>
            <div className="flex items-center justify-center gap-2 py-2">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <Heart size={32} className={ratingScore >= star ? 'fill-current' : 'stroke-current'} />
                </motion.button>
              ))}
            </div>
            <div className="flex gap-2">
              ...
              >
                {submitting ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white/40 border-t-white" /> : <Send size={16} />}
              </motion.button>
            </div>
            ...
            {orderedRatings.length > 0 && (
              <div className="space-y-3 pt-1">
                {orderedRatings.map((rating) => {
                  ...
                  return (
                    <div key={rating.id} className={`flex items-center gap-3 p-4 rounded-2xl border-2 ${isMine ? 'bg-pink-100/20 border-pink-100/30' : 'bg-indigo-100/10 border-indigo-100/20'}`}>
                      <div className={`w-12 h-12 rounded-2xl bg-white flex items-center justify-center shadow-sm ${isMine ? 'text-pink-500' : 'text-indigo-500'}`}>
                        <Star size={24} className="fill-current" />
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-black text-gray-700">{isMine ? '我的评分' : 'TA 的评分'}</span>
                          <div className="flex gap-0.5">
                            {[1, 2, 3, 4, 5].map(s => (
                              <Heart key={s} size={14} className={s <= rating.score ? (isMine ? 'text-pink-500 fill-pink-500' : 'text-indigo-400 fill-indigo-400') : 'text-gray-200'} />
                            ))}
                          </div>
                        </div>
                        <div className="mt-1.5 relative">
                          <div className="absolute -left-2 top-0 text-gray-200 font-serif text-2xl opacity-50">“</div>
                          <p className="text-[11px] text-gray-500 font-bold pl-2 pr-2 italic">{rating.note || '今天也是超级爱你的一天～'}</p>
                          <div className="absolute -right-1 bottom-0 text-gray-200 font-serif text-2xl opacity-50">”</div>
                        </div>
                      </div>
                    </div>
                  );
            ...
            {!partnerRating && myRating && (
              <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-100 rounded-3xl bg-gray-50/30">
                ...
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] italic">等待 TA 的心动回应</p>
              </div>
            )}
          </div>
        </motion.div>
...
function MomentStatusRow({
  ...
  const config = person?.momentStatus ? momentStatusMap[person.momentStatus] : null;
    ...
    >
      <div className="relative size-12 shrink-0">
        <div className="size-10 overflow-hidden rounded-full border-[3px] border-white shadow-md">
          <AppImage src={avatar} alt={fallbackName} className="size-full object-cover" width={80} height={80} crop="square" sizes="40px" />
        </div>
        <span className={`absolute bottom-0 right-0 flex size-5 items-center justify-center rounded-full border-2 border-white text-white shadow-sm ${accent.icon}`}>
          {editable ? <Heart size={10} className="fill-white" /> : <Sparkles size={9} />}
        </span>
      </div>
      <div className="min-w-0 flex-1 overflow-hidden">
        <div className="flex min-w-0 items-center gap-2">
          <p className="truncate text-sm font-black text-gray-800">{person?.username || fallbackName}</p>
          {timeLabel && <span className="max-w-[56px] shrink-0 truncate text-[9px] font-bold text-gray-300">{timeLabel}</span>}
        </div>
        <p className="mt-1 truncate text-xs font-bold text-gray-500">{statusText}</p>
      </div>
      <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-black ${editable ? `shadow-lg ${accent.button}` : 'bg-gray-100 text-gray-400'}`}>
        <ActionIcon size={14} className={tone === 'pink' ? 'fill-current' : ''} />
      </span>
    </Wrapper>
  );
}
