...
function logToDraft(log?: PeriodDailyLog): LogDraft {
  ...
  return {
    ...
    temperatureCelsius:
      log.temperatureCelsius === undefined
        ? ''
        : String(log.temperatureCelsius),
...
function statusText(
  ...
  if (
    settings?.mode === 'trying_to_conceive' &&
    summary.predictedOvulationDate
  ) {
    return {
...
export default function PeriodAssistantPage({
  ...
  const latestOpenRecord = useMemo(
    () => records.find((record) => !record.endDate),
    [records]
  )
  const selectedLog = logByDate.get(selectedDate)
  const status = useMemo(
    () => statusText(summary, normalizedSettings),
  ...
  const saveDailyLog = async () => {
    ...
    try {
      await onSaveLog(selectedDate, {
        ...
        temperatureCelsius: logDraft.temperatureCelsius
          ? Number(logDraft.temperatureCelsius)
          : undefined,
  ...
  const syncCareTodos = async () => {
    ...
    try {
      ...
      setSyncText(
        result.skipped
          ? '当前没有需要同步的新提醒。'
          : `已同步 ${result.created + result.updated} 条提醒待办。`
      )
    } finally {
      setBusy(false)
    }
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-full px-5 pb-10"
    >
      <header className="sticky top-0 z-30 mb-4 flex items-center justify-between gap-3 bg-[#FEF9F3]/95 pb-3 pt-6 backdrop-blur">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h1 className="truncate text-xl font-black text-gray-800">
            姨妈健康
          </h1>
        </div>
        {isLoading ? (
      ...
      {activeView === 'tracker' && (
        <div className="space-y-5">
          <section
            className={`relative overflow-hidden rounded-[30px] ${status.tone} p-5 text-white shadow-xl shadow-rose-100`}
          >
            ...
            <div className="relative pr-14">
              ...
              <p className="mt-4 text-[11px] font-bold leading-relaxed text-white/65">
                {summary?.disclaimer || '仅供生活记录和周期参考。'}
              </p>
            </div>
            <div className="relative mt-6 grid grid-cols-3 gap-2">
              <Metric
                label="预计经期"
                value={formatShortDate(summary?.predictedStartDate)}
              />
              <Metric
                label="平均周期"
                value={`${summary?.averageCycleDays || normalizedSettings.defaultCycleDays} 天`}
              />
              <Metric
                label="已记录"
                value={`${summary?.recordCount || 0} 次`}
              />
            </div>
          </section>
            ...
            <section className="rounded-[30px] bg-white/70 p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3
                    className={`mt-1 text-base font-black bg-gradient-to-r ${currentPhase.color} bg-clip-text text-transparent`}
                  >
                    {currentPhase.name}{' '}
                ...
                <span
                  className={`rounded-2xl ${currentPhase.bgLight} ${currentPhase.textColor} px-3 py-1 text-xs font-black`}
                >
              ...
              <div className="mt-5 relative">
                <div className="h-3 w-full rounded-full bg-gray-100 flex overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${((summary?.averagePeriodDays || 5) / (summary?.averageCycleDays || 28)) * 100}%`
                    }}
                    title="月经期"
                  />
                  <div
                    ...
                    style={{
                      width: `${(Math.max(1, (summary?.averageCycleDays || 28) - 14 - 5 - (summary?.averagePeriodDays || 5)) / (summary?.averageCycleDays || 28)) * 100}%`
                    }}
                    title="卵泡期"
                  />
                  <div
                    ...
                    style={{
                      width: `${(7 / (summary?.averageCycleDays || 28)) * 100}%`
                    }}
                    title="排卵易孕期"
                  />
                  <div
                    ...
                    style={{
                      width: `${(Math.max(1, 14 - 1 - 1) / (summary?.averageCycleDays || 28)) * 100}%`
                    }}
                    title="黄体期"
                  />
                </div>
                ...
                <div
                  ...
                  style={{
                    left: `${Math.min(100, Math.max(0, (((summary?.currentCycleDay || 1) - 0.5) / (summary?.averageCycleDays || 28)) * 100))}%`
                  }}
                >
              ...
              <div className="mt-3.5 flex justify-between text-[9px] font-black text-gray-400">
                <span
                  className={
                    currentPhase.index === 0 ? 'text-rose-500 font-black' : ''
                  }
                >
                  月经期
                </span>
                <span
                  className={
                    currentPhase.index === 1 ? 'text-amber-600 font-black' : ''
                  }
                >
                  卵泡期
                </span>
                ...
                <span
                  className={
                    currentPhase.index === 3 ? 'text-purple-500 font-black' : ''
                  }
                >
                  黄体期
                </span>
          ...
          <section className="grid grid-cols-3 gap-2">
            <ActionButton
              icon={Plus}
              title={records.length === 0 ? '记录第一次' : '今天来了'}
              description="设置为开始日期"
              disabled={busy || Boolean(latestOpenRecord)}
              onClick={markStartedToday}
            />
            <ActionButton
              icon={Check}
              title="今天结束"
              description={latestOpenRecord ? '补上结束日期' : '无进行中经期'}
              disabled={busy || !latestOpenRecord}
              onClick={markEndedToday}
            />
            <ActionButton
              icon={ClipboardCheck}
              title="同步提醒"
              description="情侣关怀与待办"
              disabled={busy}
              onClick={syncCareTodos}
            />
          </section>
          {syncText && (
            <p className="rounded-2xl bg-white/70 px-4 py-3 text-xs font-bold text-rose-500">
              {syncText}
            </p>
          )}
                ...
                {calendarDays.map((date) => {
                  ...
                  const getPeriodEndDate = (rec: PeriodRecord) => {
                    ...
                    const length =
                      ...
                      normalizedSettings.defaultPeriodDays ||
                      5
                    return addDays(rec.startDate, Math.min(length, 10) - 1)
                  }
                  const isRecordedPeriod = records.some((record) =>
                    inRange(value, record.startDate, getPeriodEndDate(record))
                  )
                  const isPredictedPeriod = inRange(
                    value,
                    summary?.predictedStartDate,
                    summary?.predictedEndDate
                  )
                  const isFertile = inRange(
                    value,
                    summary?.fertileWindow?.startDate,
                    summary?.fertileWindow?.endDate
                  )
                  return (
                    <button
                      key={value}
                    ...
                    >
                      {date.getDate()}
                      {isToday && (
                        <span className="absolute -top-0.5 -right-0.5 rounded-full bg-amber-300 px-1 text-[6px] text-amber-900 leading-none">
                          今
                        </span>
                      )}
                      {hasLog && (
                        <span
                          className={`absolute bottom-0.5 h-1 w-1 rounded-full ${isSelected || isRecordedPeriod ? 'bg-white' : 'bg-rose-500'}`}
                        />
                      )}
                    </button>
                  )
              ...
              <div>
                <h3 className="text-sm font-black text-gray-800">
                  {selectedDate === todayString()
                    ? '今日健康状态'
                    : `${formatShortDate(selectedDate)} 健康状态`}
                </h3>
            ...
            {isLogEditing ? (
              ...
              <div className="mt-4 space-y-3.5">
                {selectedLog.flow && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="rounded-full bg-rose-50 px-2.5 py-1 text-[10px] font-black text-rose-500">
                      {flowOptions.find((o) => o.value === selectedLog.flow)
                        ?.label || selectedLog.flow}
                    </span>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-2xl bg-rose-50/40 p-2.5">
                    ...
                    <p className="mt-1 text-sm font-black text-rose-500">
                      {selectedLog.painLevel !== undefined
                        ? `${selectedLog.painLevel} / 5`
                        : '--'}
                    </p>
                  </div>
                  <div className="rounded-2xl bg-rose-50/40 p-2.5">
                    ...
                    <p className="mt-1 text-sm font-black text-gray-700">
                      {selectedLog.energyLevel !== undefined
                        ? `${selectedLog.energyLevel} / 5`
                        : '--'}
                    </p>
                  </div>
                </div>
                ...
                {selectedLog.symptoms && selectedLog.symptoms.length > 0 && (
                  <div>
                    <p className="text-[10px] font-black text-gray-400 mb-1.5">
                      记录症状
                    </p>
                    <div className="flex flex-wrap gap-1.5">
                      {selectedLog.symptoms.map((s: string) => (
                        <span
                          key={s}
                          className="rounded-full bg-rose-50 px-2.5 py-1 text-[10px] font-bold text-rose-500"
                        >
                          {s}
                        </span>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {selectedLog.moods.map((m: string) => (
                        <span
                          key={m}
                          className="rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-bold text-blue-500"
                        >
                          {m}
                        </span>
                      ))}
                ...
                {(selectedLog.temperatureCelsius ||
                  ...
                  <div className="border-t border-dashed border-rose-100/50 pt-3 mt-1 grid grid-cols-2 gap-2">
                    ...
                    {selectedLog.lhTestResult && (
                      <div className="text-[10px] font-bold text-gray-500">
                        ...
                        <span className="font-black text-emerald-600">
                          {lhOptions.find(
                            (o) => o.value === selectedLog.lhTestResult
                          )?.label || selectedLog.lhTestResult}
                        </span>
                      </div>
                    )}
                    {selectedLog.intercourse && (
                ...
                {selectedLog.note && (
                  <div className="border-t border-dashed border-rose-100/50 pt-3 mt-1">
                    ...
                    <p className="text-xs font-bold leading-relaxed text-gray-600 bg-rose-50/20 p-2.5 rounded-2xl whitespace-pre-wrap">
                      {selectedLog.note}
                    </p>
                  </div>
                )}
          ...
          <section className="rounded-[28px] bg-white/70 p-4 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-2">
              <Stepper
                label="默认周期"
                value={normalizedSettings.defaultCycleDays}
                min={15}
                max={90}
                onChange={(value) =>
                  onUpdateSettings({ defaultCycleDays: value })
                }
              />
              <Stepper
                label="默认经期"
                value={normalizedSettings.defaultPeriodDays}
                min={1}
                max={15}
                onChange={(value) =>
                  onUpdateSettings({ defaultPeriodDays: value })
                }
              />
              <Stepper
                label="提前提醒"
                value={normalizedSettings.reminderLeadDays}
                min={0}
                max={14}
                onChange={(value) =>
                  onUpdateSettings({ reminderLeadDays: value })
                }
              />
            </div>
          </section>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs font-bold leading-relaxed text-gray-500">
                    预计排卵日{' '}
                    {formatShortDate(summary?.predictedOvulationDate)}，参考窗口{' '}
                    {formatShortDate(summary?.fertileWindow?.startDate)} -{' '}
                    {formatShortDate(summary?.fertileWindow?.endDate)}。
                  </p>
                </div>
              </div>
            </section>
              ...
              <div className="space-y-2.5 max-h-[300px] overflow-y-auto scrollbar-hide">
                {records.length === 0 ? (
                  ...
                  records.map((record) => (
                    <article
                      key={record.id}
                      className="rounded-2xl bg-white/80 p-3 shadow-xs border border-rose-50/30"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="mt-0.5 text-[9px] font-bold text-gray-400">
                            {durationText(record)} · 疼痛{' '}
                            {record.painLevel ?? 0}/5
                          </p>
                        </div>
                      ...
                      {record.note && (
                        <p className="mt-2 text-[10px] font-bold leading-relaxed text-gray-500 bg-rose-50/20 p-2 rounded-xl">
                          {record.note}
                        </p>
                      )}
                    </article>
        ...
        <div className="space-y-5">
          ...
          <section className="grid grid-cols-2 gap-3">
            <TrendCard
              label="平均周期"
              value={`${summary?.averageCycleDays || normalizedSettings.defaultCycleDays} 天`}
              detail={`波动 ${summary?.cycleVarianceDays || 0} 天`}
              icon={Activity}
              color="bg-rose-50 text-rose-500"
            />
            <TrendCard
              label="平均经期"
              value={`${summary?.averagePeriodDays || normalizedSettings.defaultPeriodDays} 天`}
              detail={`${summary?.recordCount || 0} 次区间记录`}
              icon={Droplets}
              color="bg-pink-50 text-pink-500"
            />
            <TrendCard
              label="日志天数"
              value={`${summary?.loggedDayCount || logs.length} 天`}
              detail="最近最多展示 180 天"
              icon={ClipboardCheck}
            ...
            <TrendCard
              label="当前周期"
              value={`${summary?.currentCycleDay || '--'} 天`}
              detail={summary?.isInPeriod ? '正在经期中' : '非经期记录'}
              icon={CalendarDays}
            ...
            <section className="rounded-[28px] bg-white/70 p-5 shadow-sm">
              ...
              <div className="flex h-36 items-end justify-between gap-3 px-2 pt-4">
                {periodDurationHistory.map((item, index) => {
                  const maxDays =
                    Math.max(...periodDurationHistory.map((d) => d.days), 7) ||
                    7
                  ...
                  return (
                    ...
                    >
                      <div className="relative w-full flex justify-center">
                        <span className="absolute -top-7 scale-0 group-hover:scale-100 transition-all rounded-lg bg-accent px-2 py-0.5 text-[9px] font-black text-on-accent whitespace-nowrap shadow-md z-10">
                          {item.days} 天
                        </span>
                      </div>
                      <div
          ...
          <section className="rounded-[28px] bg-white/70 p-5 shadow-sm">
            ...
            <div className="space-y-4">
              {summary?.symptomStats?.length ? (
                (() => {
                  const maxCount =
                    Math.max(...summary.symptomStats.map((s) => s.count)) || 1
                    ...
                    return (
                      <div key={item.name}>
                        <div className="mb-1 flex items-center justify-between text-xs font-black text-gray-600">
                          <span>{item.name}</span>
                          <span className="rounded-full bg-rose-50 px-2.5 py-0.5 text-[9px] font-black text-rose-500">
                            {item.count} 次
                          </span>
                        </div>
          ...
          <section className="rounded-[28px] bg-white/70 p-5 shadow-sm">
            ...
            <p className="text-[10px] font-bold text-gray-400 mb-2 leading-relaxed">
              快捷生成气泡：点击快速为伴侣/自己生成温和、周全的照顾话语及妙招：
            </p>
              ...
              {[
                {
                  label: '🌿 缓解痛经妙招',
                  prompt:
                    '她这几天有些痛经，请写几句温柔的关怀话语，并提供 3 个舒缓痛经的日常小妙招。不要涉及医疗诊断。'
                },
                {
                  label: '🍵 经期热饮做法',
                  prompt:
                    '女生姨妈期适合喝什么温热饮品？请给我 3 个简单快手的热饮做法（如红糖姜茶、玫瑰花茶等）。'
                },
                {
                  label: '❤️ 关心敏感低落',
                  prompt:
                    '她在经期情绪有些低落和敏感，我该怎么说和做，才能让她觉得被陪伴和理解？请写几个具体的关心话术。'
                },
                {
                  label: '👶 备孕期放松指南',
                  prompt:
                    '我们在备孕阶段，感觉最近情绪有点紧绷。请帮我写一段温柔的放松指南，适合两个人一起读，缓解焦虑。'
                }
              ].map((item) => (
                <button
                  key={item.label}
            ...
            <button
              ...
              className="mt-2 flex min-h-10 w-full items-center justify-center gap-2 rounded-2xl bg-accent text-xs font-black text-on-accent disabled:opacity-50"
            >
              {aiCareLoading ? (
            ...
            {aiCareResult && (
              <div className="mt-3 rounded-2xl bg-rose-50 p-3">
                <p className="whitespace-pre-wrap text-xs font-bold leading-relaxed text-gray-600">
                  {aiCareResult}
                </p>
                <button
                  type="button"
                  onClick={() => {
                    ...
                    setLogDraft((current) => ({
                      ...current,
                      note: [current.note, aiCareResult]
                        .filter(Boolean)
                        .join('\n')
                    }))
...
function ActionButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="min-h-[104px] rounded-[24px] bg-white/70 p-4 text-left shadow-sm transition-colors disabled:opacity-45"
    >
    ...
    <div className="rounded-2xl bg-rose-50/70 p-3 text-center">
      ...
      <div className="mt-2 flex items-center justify-center gap-2">
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-full bg-white text-rose-500"
        >
          -
        </button>
        <span className="min-w-6 text-sm font-black text-gray-800">
          {value}
        </span>
        <button
          type="button"
...
function RecordEditor({
  ...
  return (
    <div className="space-y-4">
      ...
      <FlowPicker
        value={draft.flow}
        onChange={(flow) => setDraft({ ...draft, flow })}
      />
      <RangeField
        label="疼痛程度"
        value={draft.painLevel}
        onChange={(painLevel) => setDraft({ ...draft, painLevel })}
      />
      <ChipPicker
        title="症状"
        options={symptomOptions}
        value={draft.symptoms}
        onChange={(symptoms) => setDraft({ ...draft, symptoms })}
      />
      <textarea
        value={draft.note}
        onChange={(event) => setDraft({ ...draft, note: event.target.value })}
        placeholder="备注一点这次周期的状态..."
        rows={3}
        className="w-full resize-none rounded-2xl border border-rose-100 bg-white px-3 py-3 text-xs font-bold text-gray-700 outline-none placeholder:text-gray-300 focus:ring-2 focus:ring-rose-100"
      />
      <button
        type="button"
        onClick={onSave}
        disabled={busy || !draft.startDate}
        className="flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-rose-500 text-sm font-black text-white shadow-lg shadow-rose-100 disabled:opacity-50"
      >
        <Save size={16} />
        保存经期记录
      </button>
  ...
  return (
    <div className="space-y-4">
      ...
      <RangeField
        ...
        onChange={(energyLevel) => setDraft({ ...draft, energyLevel })}
      />
      <ChipPicker
        title="症状"
        options={symptomOptions}
      ...
      <ChipPicker
        ...
        options={moodOptions}
        value={draft.moods}
        onChange={(moods) => setDraft({ ...draft, moods })}
      />
      {showFertility && (
        <div className="grid grid-cols-2 gap-3">
          <label className="block text-xs font-black text-gray-500">
            ...
            <input
              ...
              placeholder="36.50"
              className="mt-2 w-full rounded-2xl border border-emerald-100 bg-white px-3 py-3 text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-emerald-100"
            />
          </label>
          <label className="block text-xs font-black text-gray-500">
            LH 试纸
            <select
              value={draft.lhTestResult}
              onChange={(event) =>
                setDraft({
                  ...draft,
                  lhTestResult: event.target.value as LogDraft['lhTestResult']
                })
              }
              className="mt-2 w-full rounded-2xl border border-emerald-100 bg-white px-3 py-3 text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-emerald-100"
            >
          ...
          <button
            type="button"
            onClick={() =>
              setDraft({ ...draft, intercourse: !draft.intercourse })
            }
            className={`col-span-2 min-h-11 rounded-2xl text-xs font-black ${draft.intercourse ? 'bg-emerald-500 text-white' : 'bg-emerald-50 text-emerald-700'}`}
          >
      ...
      <div className="grid grid-cols-[0.8fr_1.2fr] gap-3">
        <button
          ...
          className="min-h-12 rounded-2xl bg-white text-sm font-black text-gray-500 shadow-sm disabled:opacity-45"
        >
          清空
        </button>
        <button
          ...
          onClick={onSave}
          disabled={busy}
          className="flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-accent text-sm font-black text-on-accent disabled:opacity-50"
        >
          <Save size={16} />
          保存日志
        </button>
  ...
  return (
    <label className="block text-xs font-black text-gray-500">
      ...
      <input
        ...
        className="mt-2 w-full rounded-2xl border border-rose-100 bg-white px-3 py-3 text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-rose-100"
      />
    </label>
  )
}
      ...
      <div className="grid grid-cols-5 gap-2">
        {flowOptions.map((option) => (
          <button
            ...
            className={`min-h-10 rounded-2xl px-1 text-[10px] font-black transition-colors ${value === option.value ? 'bg-rose-500 text-white shadow-md shadow-rose-100' : 'bg-rose-50 text-rose-500'}`}
          >
            {option.label}
          </button>
        ))}
      ...
      <div className="flex flex-wrap gap-2">
        {options.map((option) => (
          <button
            ...
            className={`rounded-full px-3 py-2 text-[10px] font-black transition-colors ${value.includes(option) ? 'bg-rose-500 text-white shadow-md shadow-rose-100' : 'bg-rose-50 text-rose-500'}`}
          >
            {option}
          </button>
        ))}
  ...
  return (
    <div className="rounded-[24px] bg-white/70 p-4 shadow-sm flex items-start justify-between gap-3">
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-2 text-xl font-black text-gray-800 truncate">
          {value}
        </p>
        <p className="mt-1 text-[9px] font-bold text-gray-400 truncate">
          {detail}
        </p>
      </div>
      <div
