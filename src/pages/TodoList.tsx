...
import { CalendarDays, Check, CheckCircle2, Clock3, ImagePlus, Lightbulb, Pencil, Plus, Sparkles, Star, Trash2, X, Heart, Compass, Gift, Home, TrendingUp, HeartHandshake } from 'lucide-react';
...
const TEMPLATE_GROUPS = [
  { category: '日常', items: ['一起做早餐', '看一场老电影', '深夜散步', '交换一天歌单', '一起整理房间', '给对方写便签'] },
  { category: '旅行', items: ['坐一次火车旅行', '去海边看日出', '去陌生城市住一晚', '拍一组旅行合照', '一起逛当地夜市', '盖一次纪念章'] },
  { category: '约会', items: ['去游乐园', '一起看电影', '吃一次烛光晚餐', '去咖啡店待一下午', '一起做手工', '穿情侣装出门'] },
  { category: '纪念', items: ['复刻第一次约会', '拍周年照', '写一封未来信', '做一本回忆相册', '交换纪念礼物', '记录一段视频告白'] },
  { category: '成长', items: ['一起运动7天', '学一道新菜', '读同一本书', '存一笔旅行基金', '完成一次断舍离', '一起做年度计划'] },
];

type TodoDraft = Pick<TodoItem, 'title' | 'category'> & Partial<Pick<TodoItem, 'description' | 'targetDate' | 'isFeatured' | 'sortOrder' | 'completedAt' | 'memoryNote' | 'memoryImages'>>;
...
const displayDate = (value?: string) => (value ? new Date(value).toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' }) : '');

const displayDateTime = (value?: string) => {
  if (!value) return '';
  const date = new Date(value);
  if (isNaN(date.getTime())) return '';
  const hasTime = value.includes('T') && !value.endsWith('T00:00:00') && !value.endsWith('T00:00:00.000Z') && !value.endsWith('T00:00:00.000') && !value.endsWith('T00:00:00Z');
  if (hasTime) {
...
export default function TodoList({
  ...
  const [completionDate, setCompletionDate] = useState(() => getLocalDateTimeString());
  ...
  const openAdd = (template?: TodoDraft) => {
    ...
    if (selected) {
      setSelected(null);
      setTimeout(() => {
        setDraft({ ...emptyDraft(), targetDate: getLocalDateTimeString(), ...template });
    ...
    if (selected) {
      ...
      setTimeout(() => {
        ...
        setDraft({
          ...
          targetDate: todo.targetDate ? getLocalDateTimeString(todo.targetDate) : '',
          isFeatured: Boolean(todo.isFeatured),
          sortOrder: todo.sortOrder,
          completedAt: todo.completedAt ? getLocalDateTimeString(todo.completedAt) : '',
  ...
  const closeEditorModal = useModalHistory('todo-editor', isEditorOpen, closeEditor);
  const closeDetailModal = useModalHistory('todo-detail', Boolean(selected), () => setSelected(null));
  const closeCompletionModal = useModalHistory('todo-completion', Boolean(completionTodo), () => setCompletionTodo(null));
  const closeTemplateModal = useModalHistory('todo-templates', showTemplate, () => setShowTemplate(false));
  const closeAiIdeasPanel = useModalHistory('todo-ai-ideas', aiIdeasOpen, () => setAiIdeasOpen(false));
  ...
  const submitDraft = async () => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : '保存失败，请稍后再试', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const addTemplate = async (title: string, category: string) => {
    setBusyId(title);
    try {
      await onAdd({ title, category, description: '', isFeatured: false, sortOrder: data.length });
      showToast('灵感已加入清单', 'success');
    } catch (error) {
      console.error('Add template todo failed:', error);
      showToast(error instanceof Error ? error.message : '添加失败，请稍后再试', 'error');
    } finally {
      setBusyId(null);
    }
  };
  ...
  const generateAiIdeas = async () => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : 'AI 灵感生成失败，请稍后重试', 'error');
    } finally {
  ...
  const startCompletion = (todo: TodoItem) => {
    if (selected) {
      ...
      setTimeout(() => {
        ...
        setCompletionDate(todo.completedAt ? getLocalDateTimeString(todo.completedAt) : getLocalDateTimeString());
  ...
  const uploadMemoryImages = async (files: File | File[] | FileList | null) => {
    ...
    setUploading(true);
    try {
      const urls = await Promise.all(filesArray.map((file) => uploadService.upload(file)));
      setMemoryImages((prev) => [...prev, ...urls]);
      showToast('照片已上传', 'success');
    } catch (error) {
      console.error('Upload memory images failed:', error);
      showToast(error instanceof Error ? error.message : '照片上传失败', 'error');
    } finally {
      setUploading(false);
    }
  };
  ...
  const submitCompletion = async () => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : '更新失败，请稍后再试', 'error');
    } finally {
      setBusyId(null);
    }
  };
  ...
  const deleteTodo = async (todo: TodoItem) => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : '删除失败，请稍后再试', 'error');
    } finally {
      setBusyId(null);
    }
  };
    ...
    <div className="flex min-h-full flex-col bg-[linear-gradient(180deg,#fff7f2_0%,#fffafd_45%,#f8fbff_100%)] lg:h-full">
      <header className="sticky top-0 z-30 shrink-0 bg-[#fff7f2]/95 px-5 pb-3 pt-7 backdrop-blur">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-pink-400">Bucket List</p>
            <h1 className="mt-1 text-2xl font-black tracking-tight text-gray-900">100件恋爱小事</h1>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowTemplate(true)} className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-amber-500 shadow-lg shadow-rose-100/50 ring-1 ring-white/80" title="灵感模板">
              <Lightbulb size={20} />
            </button>
            <button onClick={() => openAdd()} disabled={data.length >= 100} className="flex h-11 w-11 items-center justify-center rounded-2xl bg-pink-500 text-white shadow-lg shadow-pink-100 disabled:opacity-40" title="新增小事">
              <Plus size={22} />
            </button>
          </div>
        </div>
      ...
      <main className="px-5 pb-32 pt-2 lg:flex-1 lg:overflow-y-auto lg:scrollbar-hide">
        <section className="relative overflow-hidden rounded-[28px] border border-white/90 bg-white/90 p-5 shadow-xl shadow-rose-100/35">
          <div className="flex items-end justify-between">
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">已完成进度</p>
              <p className="mt-1 text-3xl font-black text-gray-900 leading-none">
                <span className="text-pink-500">{completedCount}</span>
                <span className="text-base text-gray-300 font-bold"> / 100</span>
              </p>
            </div>
            <div className="flex gap-2">
              <Stat label="剩余" value={remainingSlots} type="remaining" />
              <Stat label="精选" value={featuredCount} type="featured" />
            </div>
          </div>
        ...
        <div className="mt-5 flex gap-2 overflow-x-auto pb-1.5 scrollbar-hide">
          {STATUS_FILTERS.map((item) => {
            ...
            return (
              ...
              >
                {item.id === 'all' && <Sparkles size={12} className={isActive ? 'text-pink-300' : 'text-pink-400'} />}
                {item.id === 'active' && <Clock3 size={12} className={isActive ? 'text-sky-300' : 'text-sky-400'} />}
                {item.id === 'completed' && <CheckCircle2 size={12} className={isActive ? 'text-emerald-300' : 'text-emerald-400'} />}
                {item.id === 'featured' && <Star size={12} className={isActive ? 'fill-amber-300 text-amber-300' : 'text-amber-500 fill-amber-400/20'} />}
                {item.label}
              </button>
            );
          })}
        ...
        <div className="mt-2.5 flex gap-2 overflow-x-auto pb-1.5 scrollbar-hide">
          {['全部', ...CATEGORIES].map((category) => {
            ...
            if (isSelected) {
              if (category === '全部') {
                btnStyle = 'bg-pink-500 text-white shadow-md shadow-pink-100 border border-pink-500 scale-[1.02]';
              } else if (meta) {
                btnStyle = `${meta.colorClass} shadow-md border ${meta.borderClass} scale-[1.02]`;
              }
            } else {
              btnStyle = 'bg-white/95 text-gray-500 border border-white/80 hover:bg-white hover:text-gray-700';
            }
            ...
            return (
              ...
              >
                {category === '全部' ? (
                  <Heart size={12} className={isSelected ? 'fill-white text-white' : 'text-pink-400'} />
                ) : meta ? (
                  <meta.icon size={12} className={isSelected ? meta.textClass : 'text-gray-400'} />
                ) : null}
                {category}
              </button>
            );
        ...
        {aiIdeasOpen && (
          <section className="mb-4 mt-3 rounded-[26px] border border-pink-100 bg-gradient-to-br from-pink-50/60 to-rose-50/30 p-4.5 shadow-sm">
            <div className="mb-3 flex items-center justify-between">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-pink-400">AI Ideas</p>
                <h2 className="text-sm font-black text-gray-900">恋爱小事灵感</h2>
              </div>
              <button
                type="button"
                onClick={generateAiIdeas}
                disabled={aiIdeasLoading}
                className="rounded-2xl bg-gray-900 px-3 py-2 text-[10px] font-black text-white hover:bg-gray-800 transition-colors disabled:opacity-50"
              >
                {aiIdeasLoading ? '生成中...' : '换一组'}
              </button>
            </div>
            {aiIdeasLoading && <p className="rounded-2xl bg-white/80 p-3 text-xs font-bold text-pink-500 border border-pink-100/50">AI 正在想几个甜甜的小任务...</p>}
            {!aiIdeasLoading && aiIdeas.length > 0 && (
              <div className="space-y-2">
                {aiIdeas.map((idea) => (
                  <div key={idea} className="flex items-center gap-2 rounded-2xl bg-white/70 border border-pink-100/30 p-2 pl-3">
                    <p className="min-w-0 flex-1 truncate text-xs font-bold text-gray-700">{idea}</p>
                    <button
                      type="button"
        ...
        {isLoading && filteredTodos.length === 0 && <EmptyState title="正在加载小事" note="把清单铺开，马上就好。" loading />}
        {!isLoading && filteredTodos.length === 0 && <EmptyState title="这里还没有小事" note="从灵感模板里挑一个，或写下你们自己的约定。" />}

        <div className="space-y-3">
          {filteredTodos.map((todo, index) => {
            const absoluteIndex = data.findIndex((item) => item.id === todo.id) + 1;
              ...
              >
                ...
                <div className="flex items-start gap-3.5">
                  ...
                  >
                    {busyId === todo.id ? (
                      ...
                      <Heart size={18} className="fill-white stroke-[2.5] stroke-white animate-pulse" />
                    ) : (
                      <Heart
                        size={18}
                ...
                {todo.completed && todo.memoryNote && (
                  <div className="mt-3 rounded-2xl bg-amber-50/30 p-3 border-l-2 border-amber-300 text-xs font-bold leading-relaxed text-gray-600">
                    <span className="mr-1 text-[11px] text-amber-500 select-none">✍️</span>
      ...
      {selected && (
        <DetailModal todo={selected} busy={busyId === selected.id} onClose={closeDetailModal} onEdit={() => openEdit(selected)} onComplete={() => startCompletion(selected)} onToggle={() => toggleTodo(selected)} onDelete={() => deleteTodo(selected)} />
      )}
      ...
      {showTemplate && <TemplateDrawer onClose={closeTemplateModal} onAdd={addTemplate} busyId={busyId} />}
    </div>
  );
}

function Stat({ label, value, type }: { label: string; value: number; type: 'remaining' | 'featured' }) {
  const isFeatured = type === 'featured';
  return (
    <div className={`rounded-2xl p-2 px-3 transition-all text-left flex items-center gap-2 ${
      isFeatured 
        ? 'bg-amber-50/60 border border-amber-100/50 text-amber-900' 
        : 'bg-rose-50/60 border border-rose-100/50 text-rose-900'
    }`}>
      <div className={`p-1.5 rounded-xl ${isFeatured ? 'bg-amber-100/80 text-amber-500' : 'bg-rose-100/80 text-rose-500'}`}>
        {isFeatured ? <Star size={12} className="fill-amber-500" /> : <Clock3 size={12} />}
      </div>
      <div>
        <p className="text-[9px] font-black uppercase tracking-wider text-gray-400 leading-none">{label}</p>
...
function EmptyState({ title, note, loading = false }: { title: string; note: string; loading?: boolean }) {
  return (
    <div className="mt-6 flex h-64 flex-col items-center justify-center rounded-[32px] border-2 border-dashed border-pink-100 bg-white/50 backdrop-blur-xs px-8 text-center transition-all duration-300">
      <div className={`mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-pink-50 to-rose-50 text-pink-400 shadow-sm ${loading ? 'animate-pulse' : ''}`}>
        {loading ? <Sparkles size={24} className="animate-spin text-pink-400" /> : <Heart size={24} className="fill-pink-300 text-pink-400" />}
      </div>
      <p className="text-sm font-black text-gray-700">{title}</p>
      <p className="mt-2 max-w-xs text-xs font-bold leading-relaxed text-gray-400">{note}</p>
    </div>
  );
}
...
function TodoEditor({
  ...
  return (
    <ModalFrame onClose={onClose}>
      <h2 className="text-xl font-black text-gray-900">{editing ? '编辑小事' : '新增小事'}</h2>
      <div className="mt-5 max-h-[70vh] space-y-3 overflow-y-auto p-1.5 scrollbar-hide">
        ...
        >
          <span className="flex items-center gap-2">
            <Star size={17} className={draft.isFeatured ? 'fill-amber-400' : ''} /> 标记为精选
          </span>
        ...
        {editing?.completed && (
          <div className="mt-4 pt-4 border-t border-gray-100 space-y-3">
            <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest text-left">完成回忆编辑</h3>
            <div className="text-left">
              <label className="block text-[10px] font-black text-gray-400 mb-1">完成时间</label>
              <input
                type="datetime-local"
            ...
            <div className="text-left">
              <label className="block text-[10px] font-black text-gray-400 mb-1">回忆感悟</label>
              <textarea
                value={draft.memoryNote || ''}
                onChange={(event) => setDraft({ ...draft, memoryNote: event.target.value })}
                placeholder="这一刻有什么想记住的？"
                className="field min-h-20 resize-none"
              />
            </div>
            <div className="text-left">
              <label className="block text-[10px] font-black text-gray-400 mb-1">回忆照片</label>
              <Upload
                multiple
...
function DetailModal({
  ...
  return (
    <ModalFrame onClose={onClose}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-pink-400">{todo.category}</p>
          <h2 className="mt-1 text-xl font-black text-gray-900">{todo.title}</h2>
        </div>
        {todo.isFeatured && <Star size={20} className="fill-amber-400 text-amber-400" />}
      </div>
      {todo.description && (
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-xs font-black text-gray-500">
        <Info icon={<CalendarDays size={15} />} label="计划" value={todo.targetDate ? displayDateTime(todo.targetDate) : '未设置'} />
        <Info icon={<Clock3 size={15} />} label="状态" value={todo.completed ? '已完成' : '进行中'} />
      </div>
      {todo.completed && (
        <section className="mt-4 rounded-[24px] bg-emerald-50 p-4 max-h-48 overflow-y-auto scrollbar-hide text-left">
          <p className="text-xs font-black text-emerald-700">完成回忆 {todo.completedAt ? ` · ${displayDateTime(todo.completedAt)}` : ''}</p>
          ...
          {!!todo.memoryImages?.length && (
            <div className="mt-3 grid grid-cols-3 gap-2">
              {todo.memoryImages.map((src) => (
                <AppImage key={src} src={src} alt="" className="aspect-square rounded-2xl object-cover" width={240} height={240} crop="square" sizes="33vw" />
              ))}
            </div>
          )}
        </section>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        <button onClick={onEdit} className="rounded-2xl bg-gray-100 py-3 text-xs font-black text-gray-600">
...
function CompletionModal({
  ...
  return (
    <ModalFrame onClose={onClose}>
      <p className="text-[10px] font-black uppercase tracking-widest text-emerald-500">完成回忆</p>
      <h2 className="mt-1 text-xl font-black text-gray-900">{todo.title}</h2>
...
function TemplateDrawer({ onClose, onAdd, busyId }: { onClose: () => void; onAdd: (title: string, category: string) => void; busyId: string | null }) {
  return (
    <ModalFrame onClose={onClose} wide>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-amber-500">Ideas</p>
      ...
      <div className="mt-5 max-h-[62vh] space-y-5 overflow-y-auto p-1.5 scrollbar-hide">
        {TEMPLATE_GROUPS.map((group) => (
          <section key={group.category}>
            <h3 className="mb-2 text-xs font-black text-gray-500">{group.category}</h3>
            <div className="grid grid-cols-2 gap-2">
              {group.items.map((item) => (
                <button key={item} onClick={() => onAdd(item, group.category)} disabled={busyId === item} className="rounded-2xl bg-gray-50 p-3 text-left text-xs font-black leading-relaxed text-gray-700 ring-1 ring-gray-100 transition-all hover:bg-pink-50 hover:text-pink-600 disabled:opacity-40">
...
function Info({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-2 rounded-2xl bg-gray-50 p-3">
      ...
      <span><span className="text-gray-400">{label}</span> · {value}</span>
    </div>
  );
}

function ModalFrame({ children, onClose, wide = false }: { children: ReactNode; onClose: () => void; wide?: boolean }) {
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center p-4">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-gray-950/30 backdrop-blur-sm" onClick={onClose} />
      <motion.div initial={{ y: 120, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className={`relative max-h-[88vh] w-full ${wide ? 'max-w-md' : 'max-w-sm'} overflow-hidden rounded-[32px] bg-white p-6 shadow-2xl`}>
        <button onClick={onClose} className="absolute right-5 top-5 flex h-9 w-9 items-center justify-center rounded-2xl bg-gray-50 text-gray-400">
          <X size={18} />
        </button>
        {children}
      </motion.div>
