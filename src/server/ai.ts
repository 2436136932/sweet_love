...
export function isAiConfigured() {
  switch (env.ai.provider) {
    case "openai-compatible":
      return Boolean(
        env.ai.openaiCompatible.apiKey &&
          env.ai.openaiCompatible.baseUrl &&
          env.ai.openaiCompatible.model,
      );
    case "gemini":
...
async function withTimeout<T>(task: Promise<T>, timeoutMs: number, provider: string): Promise<T> {
  let timeout: ReturnType<typeof setTimeout> | undefined;
  const timeoutTask = new Promise<never>((_, reject) => {
    timeout = setTimeout(() => reject(new AiError(`${provider} AI request timed out after ${timeoutMs}ms`, 504, provider)), timeoutMs);
  });
...
function splitSystemMessages(messages: AiMessage[]) {
  ...
  const chatMessages = messages.filter((message) => message.role !== "system") as Array<
    AiMessage & { role: "user" | "assistant" }
  >;
  return { system, chatMessages };
}

// 兼容 OpenAI Chat Completions 协议的供应商：OpenAI、DeepSeek、通义千问兼容模式、Moonshot 等。
async function chatWithOpenAiCompatible(messages: AiMessage[], options: AiChatOptions): Promise<AiChatResult> {
  const provider = "openai-compatible";
  const config = env.ai.openaiCompatible;
  assertConfigured(Boolean(config.apiKey && config.baseUrl && config.model), provider);
  ...
  try {
    ...
    if (!res.ok) {
      const message = readErrorMessage(payload) || `OpenAI-compatible AI request failed with status ${res.status}`;
    ...
    if (error instanceof Error && error.name === "AbortError") {
      throw new AiError(`OpenAI-compatible AI request timed out after ${timeoutMs}ms`, 504, provider);
    }
    throw error;
  } finally {
...
function toGeminiContents(messages: Array<AiMessage & { role: "user" | "assistant" }>) {
...
async function chatWithGemini(messages: AiMessage[], options: AiChatOptions): Promise<AiChatResult> {
  const provider = "gemini";
  const config = env.ai.gemini;
  assertConfigured(Boolean(config.apiKey), provider);
...
function toClaudeMessages(messages: Array<AiMessage & { role: "user" | "assistant" }>): Anthropic.MessageParam[] {
  return messages.map((message) => ({
    role: message.role,
...
async function chatWithClaude(messages: AiMessage[], options: AiChatOptions): Promise<AiChatResult> {
  const provider = "claude";
  const config = env.ai.claude;
  assertConfigured(Boolean(config.apiKey), provider);
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Claude AI request failed";
    throw new AiError(message, undefined, provider);
  }
}

// 对外唯一入口：业务代码不关心 provider，只拿统一的 content/model/usage。
export async function chatWithAi(messages: AiMessage[], options: AiChatOptions = {}): Promise<AiChatResult> {
  switch (env.ai.provider) {
    case "openai-compatible":
