import { DiaryEntry, TodoItem, Message, User, AuthState, Anniversary, AlbumImage, AlbumMapData, AlbumOverview, AlbumPage, Couple, DailyRating, MenuDish, MealOrderDay, MealOrderItem, KitchenRecipe, KitchenShoppingList, KitchenShoppingListItem, KitchenCookCheckin, PeriodOverview, PeriodRecord, PeriodDailyLog, PeriodSettings, MomentStatusPayload, AiGenerateResponse, AiGenerateType } from "../types";
...
export const aiService = {
  ...
  async generate(request: { type: AiGenerateType; prompt: string; context?: unknown }): Promise<AiGenerateResponse> {
...
export const ratingService = {
  async getTodayStatus(): Promise<DailyRating[]> {
    const res = await fetch(`${API_BASE}/ratings/today`, { headers: getHeaders() });
...
export const periodService = {
  ...
  async create(record: Omit<PeriodRecord, "id" | "createdById">): Promise<PeriodOverview> {
  ...
  async update(id: string, record: Partial<Omit<PeriodRecord, "id" | "createdById">>): Promise<PeriodOverview> {
  ...
  async saveLog(date: string, log: Partial<Omit<PeriodDailyLog, "id" | "date" | "createdById">>): Promise<PeriodOverview> {
  ...
  async updateSettings(settings: Partial<PeriodSettings>): Promise<PeriodOverview> {
  ...
  async syncCareTodos(clientToday?: string): Promise<{ skipped: boolean; created: number; updated: number; predictedStartDate?: string }> {
...
export const userService = {
  async updateProfile(profile: { username?: string; avatar?: string; bio?: string }): Promise<User> {
...
export const diaryService = {
  async getAll(startDate?: string, endDate?: string): Promise<DiaryEntry[]> {
    ...
    const res = await fetch(`${API_BASE}/diaries${query ? `?${query}` : ""}`, { headers: getHeaders() });
...
export const todoService = {
  ...
  async create(todo: Pick<TodoItem, "title" | "category"> & Partial<Pick<TodoItem, "description" | "targetDate" | "isFeatured" | "sortOrder">>): Promise<TodoItem> {
...
export const messageService = {
  ...
  async send(content: string, imageUrl?: string, replyToId?: string): Promise<Message> {
...
export const albumService = {
  ...
  async getPage(params: { category?: string; limit?: number; cursor?: string; startDate?: string; endDate?: string } = {}): Promise<AlbumPage> {
    const search = new URLSearchParams();
  ...
  async getMapData(category?: string, startDate?: string, endDate?: string): Promise<AlbumMapData> {
    const search = new URLSearchParams();
    if (category) search.set("category", category);
    if (startDate) search.set("startDate", startDate);
    if (endDate) search.set("endDate", endDate);
    const query = search.toString();
    const res = await fetch(`${API_BASE}/album/map${query ? `?${query}` : ""}`, {
      headers: getHeaders(),
    });
    await ensureOk(res, "Failed to fetch album map");
    return res.json();
  },
  async getLocationMissing(category?: string, startDate?: string, endDate?: string): Promise<AlbumImage[]> {
    const search = new URLSearchParams();
    if (category) search.set("category", category);
    if (startDate) search.set("startDate", startDate);
    if (endDate) search.set("endDate", endDate);
    const query = search.toString();
    const res = await fetch(`${API_BASE}/album/location-missing${query ? `?${query}` : ""}`, {
      headers: getHeaders(),
    });
...
export const menuService = {
  async getDishes(): Promise<MenuDish[]> {
    const res = await fetch(`${API_BASE}/menu/dishes`, { headers: getHeaders() });
    await ensureOk(res, "Failed to fetch menu dishes");
    return res.json();
  },
  async createDish(dish: { name: string; category: string; description?: string; imageUrl?: string }): Promise<MenuDish> {
...
export const mealOrderService = {
  async getToday(): Promise<MealOrderDay> {
    const res = await fetch(`${API_BASE}/meal-orders/today`, { headers: getHeaders() });
    await ensureOk(res, "Failed to fetch meal order");
    return res.json();
  },
  async getByDate(date: string): Promise<MealOrderDay> {
    const res = await fetch(`${API_BASE}/meal-orders?date=${encodeURIComponent(date)}`, { headers: getHeaders() });
    await ensureOk(res, "Failed to fetch meal order");
    return res.json();
  },
  async addTodayItem(dish: Pick<MenuDish, "id" | "name" | "category" | "description" | "imageUrl">): Promise<MealOrderItem> {
  ...
  async updateItem(id: string, item: { quantity?: number; note?: string }): Promise<MealOrderItem> {
...
export const kitchenService = {
  async getRecipes(): Promise<KitchenRecipe[]> {
    const res = await fetch(`${API_BASE}/kitchen/recipes`, { headers: getHeaders() });
  ...
  async updateRecipe(id: string, recipe: Partial<KitchenRecipe>): Promise<KitchenRecipe> {
  ...
  async getShoppingList(date: string): Promise<KitchenShoppingList> {
    const res = await fetch(`${API_BASE}/kitchen/shopping-lists?date=${encodeURIComponent(date)}`, { headers: getHeaders() });
    await ensureOk(res, "Failed to fetch shopping list");
    return res.json();
  },
  async generateShoppingList(date: string, recipeIds: string[]): Promise<KitchenShoppingList> {
  ...
  async updateShoppingItem(id: string, item: { checked?: boolean; note?: string }): Promise<KitchenShoppingListItem> {
  ...
  async getCheckins(): Promise<KitchenCookCheckin[]> {
    const res = await fetch(`${API_BASE}/kitchen/checkins`, { headers: getHeaders() });
    await ensureOk(res, "Failed to fetch kitchen checkins");
    return res.json();
  },
  async createCheckin(checkin: { date: string; recipeId?: string; title: string; imageUrl?: string; note?: string; rating?: number }): Promise<KitchenCookCheckin> {
